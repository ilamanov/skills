...
async function scan(dir) {
  ...
  for (const entry of entries) {
    ...
    const contents = (await readFile(fullPath, 'utf8'))
      // anchor hrefs are user-facing links, not runtime imports
      .replace(/<a\b([^>]*)\s+href=(["'])https?:\/\/[^"']+\2/gi, '<a$1')
      // SVG/XML namespace declarations are not network fetches
      .replace(/\bxmlns(:[a-z0-9]+)?=(["'])https?:\/\/[^"']+\2/gi, '')
