...
4. **Triage each finding** into one of:
   ...
   **Migration-ordering findings are the canonical cascade trigger** and should be Skip by default. Anything of the shape "what if app code runs before its backfill / what if a row exists with default usage before the migration corrects it / restore-or-create the missing row in the request path" assumes a deploy window that doesn't exist here — migrations ship with the code and apply before the new code serves traffic. The right shape is a migration + a backfill, full stop; per-request reconciliation, in-flow seeding, or "ensure account exists" guards in business logic are the first link of the cascade. Skip them on sight rather than implementing and then unwinding later.
   ...
   If the scenario the reviewer is guarding against won't occur in this product's actual usage, classify as Skip. **Before fixing a finding, think about whether it's realistic for a product like this. If you're not sure, consult the user** — don't default to Fix just because the reviewer raised it, and don't silently Skip something the user might want addressed. This applies with particular force to backward-compatibility / migrate-existing-behavior findings: assuming there's no existing usage is usually correct for MVP/prototype code and lets you change things cleanly, but it's also exactly the kind of assumption that's expensive to get wrong — so when you can't confidently confirm there's no existing usage, treat it as a question for the user rather than a default Skip. The goal is to fix real bugs, not gold-plate the code — reasonable assumptions about how this specific app is deployed and used are explicitly allowed, but genuine uncertainty about realism (including whether existing usage must be preserved) is a question for the user, not a coin flip.

5. **Triage autonomously and report — but stop and ask whenever you're not sure.** For findings where you *have a confident position*, decide Fix vs. Skip yourself and proceed to act on them (point 6) without waiting for confirmation. **But the autonomous path is only for findings you're sure about.** If the realism check in point 4 left you genuinely unsure about a finding — most often a backward-compatibility / migrate-existing-behavior finding where you can't confirm whether existing usage must be preserved — **do not pick a side to keep the loop moving.** Neither defaulting to Fix nor defaulting to Skip is acceptable when you don't actually hold that position; both silently assume an answer the user might disagree with. Instead, **break the loop: pause and ask the user about those specific findings, and wait for their answer before acting on them.** It is entirely fine to stop here and hold — verifying with the user costs a message; guessing wrong costs a bad fix or a shipped regression. The rest of the round (findings you *are* sure about) can proceed in parallel while those wait. A status message is still posted so the user has visibility and can interject if they disagree. Format:
