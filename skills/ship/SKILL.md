...
## Standing project assumptions

This project is at MVP stage. These hold for every run unless the user explicitly overrides them in the prompt:

- **No existing users, no backward compatibility.** There is no production usage, no persisted data in any old shape, and no external callers depending on current behavior. Make schema and code changes cleanly — add, modify, rename, or remove freely; no shims, dual-writes, deprecation windows, compatibility shells, or migrations of data that doesn't exist. This settles the realism check in Step 6: backward-compatibility findings ("this is a breaking change," "preserve the old API / signature / response shape," "migrate existing rows / callers," "don't change this field/default because something might rely on it") are **Skip by default** — do **not** stop and ask whether existing usage must be preserved.
- **The database is a disposable testing branch, not prod.** It holds no production data — you may apply migrations and otherwise mutate it freely without asking. The "ask before applying to the connected DB" gate in Step 7 does **not** apply to this DB.
...
1. **Generate migrations** if the schema source changed. Use the project's standard generator in **create-only / generate** mode. Commit the generated file onto the schema-owning PR, then resubmit the stack.

   The connected DB is a disposable testing branch (see Standing project assumptions), so applying migrations to it is pre-authorized — apply via the project's standard apply command without asking whenever it's useful (e.g. the new code expects the schema, or you want to dry-run a non-trivial transformation). This authorization covers **only** the testing DB; if a run is ever pointed at a shared or production database, revert to asking the user to confirm before applying and block the merge on that confirmation.
