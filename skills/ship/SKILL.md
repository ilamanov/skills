...
4. **Triage each finding** into one of:
   ...
   **Realism check.** Before classifying as Fix, ask: is this finding actually realistic for **this product** at the **end state we're building toward**? Every finding implicitly assumes something — about scale, concurrency, deploy ordering, traffic shape, or who calls the code — and the question is whether that assumption holds here. Common ways it doesn't:
   ...
   If the scenario the reviewer is guarding against won't occur in this product's actual usage, classify as Skip. When in doubt, lean toward Skip. The goal is to fix real bugs, not gold-plate the code — reasonable assumptions about how this specific app is deployed and used are explicitly allowed.
