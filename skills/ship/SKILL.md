...
Submit the stack via Graphite — one PR per branch. **Always open every PR as ready for review — never as a draft.** PR creation auto-triggers a review on each new PR, so a draft just stalls the review loop. `gt submit --no-interactive` opens new PRs as **drafts** by default, so pass `--publish` explicitly to avoid a wasteful "mark ready for review" round-trip that delays the auto-review. Never pass `--draft` (or Graphite's draft equivalent), and don't fall back to draft for intermediate/known-broken states — including in existing-changes mode, where the pull toward draft is strongest; if a PR genuinely isn't ready, say so in chat and let the user decide rather than silently shipping a draft.
