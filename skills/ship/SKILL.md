...
- **Scope from the spec, not just the ticket.** If the ticket points at a feature spec or implementation doc in the repo (e.g. `docs/<feature>.md`), read its in-scope / out-of-scope sections and treat them as authoritative for v1 — work the doc explicitly defers to a later version stays out, even when it looks like a natural improvement or a review agent later asks for it. A run once implemented an automatic field-rename migration the repo's own doc had deferred and had to unwind it.
...
**Build the new behavior cleanly — don't add backward-compat scaffolding on your own initiative.** The Step 6 realism check (no existing usage → no compat shims, dual-writes, legacy redirects, or defensive data migrations) governs the code you *write*, not just the review findings you triage. For an MVP-stage feature with no production usage, don't proactively add a redirect from an old URL, preserve a deprecated field, or migrate data that doesn't exist — build the new thing and delete the old. If you can't confidently establish there's no existing usage to protect, that's a question for the user, not a license to scaffold defensively. (A run once invented a legacy-URL redirect route for a feature with no callers and had to remove it.)
