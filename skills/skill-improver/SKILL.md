...
- Section order deviating from the prescribed sequence (Hero → PR Stack → High-stakes → Endpoint Audit → Schema → Code Tour → File Index). There is no separate "PR Evolution" section — per-PR commit/finding history now lives on each PR card inside the PR Stack; a standalone evolution section is itself a violation.
...
- (FINAL) Inside a PR card, follow-up commits not tagged with a cause (review finding / user steering / other), or skipped review findings not surfaced in their own warning-colored row.
