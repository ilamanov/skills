...
- (FINAL) Follow-up commits on a PR card that aren't tagged with a cause (review finding / user steering / other), or skipped review findings not surfaced in a warning-colored row. (The brief skill folds per-PR commit/finding history into each PR Stack card — there is no separate "PR Evolution" section.)
