...
The script reads `state/state.json` and emits only conversations newer than the last cursor, one JSON object per line, oldest first. Each record has these keys — use them directly instead of guessing (every recent run has wasted a turn or two trying `first_prompt`/`full_prompt`, getting blanks, then dumping `.keys()` to discover the real names): `source`, `conversation_id`, `started_at`, `cwd`, `file_path`, `git_branch`, `first_user_prompt` (the field holding the opening prompt — *not* `first_prompt`), and `project`.
